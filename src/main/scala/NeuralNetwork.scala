...
object NeuralNetwork {
  ...
  val singleThreadExecutor: ExecutorService =
  ...
  val hyperparameters = Factory[
    Builtins with CNNs with L2Regularization with Adam with INDArrayLearningRate
  ...
  object weightsAndBiases {
    ...
    var loaded = false
    var cnnWeight: INDArrayWeight = INDArrayWeight(
      Nd4j
        .randn(Array(NumFilters, NumberOfChannels, KernelHeight, KernelWidth)) * WeightScale
    )
    var cnnBias: INDArrayWeight = INDArrayWeight(Nd4j.zeros(NumFilters))
    var affineWeight: INDArrayWeight = INDArrayWeight(
      Nd4j.randn(
        Array(
          NumFilters * (PixelHeight / PoolSize) * (PixelWidth / PoolSize),
          HiddenDim
        )
      ) * WeightScale
    )
    var affineBias: INDArrayWeight = INDArrayWeight(Nd4j.zeros(HiddenDim))
    var affineLastWeight: INDArrayWeight = INDArrayWeight(
      Nd4j.randn(Array(HiddenDim, NumberOfClasses)) * WeightScale
    )
    var affineLastBias: INDArrayWeight = INDArrayWeight(
      Nd4j.zeros(NumberOfClasses)
    )
  }
  ...
  def affine(input: INDArrayLayer,
  ...
  def myNeuralNetwork(input: INDArray): INDArrayLayer = {
    ...
    val affineOfAffineRuleOfCnnLayer = affine(
      affineRuleOfCnnLayer.reshape(input.shape()(0), HiddenDim),
      affineLastWeight,
      affineLastBias
    )
  ...
  class Trainer(batchSize: Int, numberOfEpochs: Int = 5) {
    ...
    def startTrain(): Unit = {
      ...
      def trainTask(next: DataSet, epoch: Int, iteration: Int): Future[Unit] = {
        ...
        lossBuffer += loss
        hyperparameters.logger.info(
          s"epoch=$epoch iteration=$iteration batchSize=$batchSize loss=$loss"
        )
      }
      ...
      while (epoch < numberOfEpochs) {
        ...
        while (iterator.hasNext) {
          Await.result(
            trainTask(iterator.next, epoch, i).toScalaFuture,
            Duration.Inf
          )
          i += 1
        }
  ...
  def getAccuracyResult: String = {
    ...
    val iterator =
      new MnistDataSetIterator(trainBatchSize, false, random.nextInt)
    while (iterator.hasNext) {
      val next = iterator.next
      val testDataBatch = next.getFeatures
      val testDataLabels = next.getLabels
      val predictResult = Await.result(
        myNeuralNetwork(testDataBatch).predict.toScalaFuture,
        Duration.Inf
      )
  ...
  def saveWeights(): Unit = {
    val cnnWeightStream = new DataOutputStream(
      new FileOutputStream(new File("data/CnnWeight.bin"))
    )
    val cnnBiasStream = new DataOutputStream(
      new FileOutputStream(new File("data/CnnBias.bin"))
    )
    val affineWeightStream = new DataOutputStream(
      new FileOutputStream(new File("data/AffineWeight.bin"))
    )
    val affineBiasStream = new DataOutputStream(
      new FileOutputStream(new File("data/AffineBias.bin"))
    )
    val affineLastWeightStream = new DataOutputStream(
      new FileOutputStream(new File("data/AffineLastWeight.bin"))
    )
    val affineLastBiasStream = new DataOutputStream(
      new FileOutputStream(new File("data/AffineLastBias.bin"))
    )
    try {
  ...
  def loadWeights(): Unit = {
    if (loaded) return
    val cnnWeightStream = new DataInputStream(
      new FileInputStream(new File("data/CnnWeight.bin"))
    )
    val cnnBiasStream = new DataInputStream(
      new FileInputStream(new File("data/CnnBias.bin"))
    )
    val affineWeightStream = new DataInputStream(
      new FileInputStream(new File("data/AffineWeight.bin"))
    )
    val affineBiasStream = new DataInputStream(
      new FileInputStream(new File("data/AffineBias.bin"))
    )
    val affineLastWeightStream = new DataInputStream(
      new FileInputStream(new File("data/AffineLastWeight.bin"))
    )
    val affineLastBiasStream = new DataInputStream(
      new FileInputStream(new File("data/AffineLastBias.bin"))
    )
    try {
  ...
  def predict(input: INDArray): Int = {
    loadWeights()
    val predictResult =
      Await.result(myNeuralNetwork(input).predict.toScalaFuture, Duration.Inf)
  ...
  def train(): Unit = {
    ...
    cnnWeight = INDArrayWeight(
      Nd4j
        .randn(Array(NumFilters, NumberOfChannels, KernelHeight, KernelWidth)) * WeightScale
    )
    cnnBias = INDArrayWeight(Nd4j.zeros(NumFilters))
    affineWeight = INDArrayWeight(
      Nd4j.randn(
        Array(
          NumFilters * (PixelHeight / PoolSize) * (PixelWidth / PoolSize),
          HiddenDim
    ...
    affineLastWeight = INDArrayWeight(
      Nd4j.randn(Array(HiddenDim, NumberOfClasses)) * WeightScale
    )
